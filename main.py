...
# Sidebar input for how many trades to display
...
- Commission is $1.5 per 1 lot per trade.
- Profit/Loss without commission is ±$1 per 0.01 lot.
- Thus, net profit if win on L lots = 98.5 × L dollars.
- Net loss if lose on L lots = -101.5 × L dollars.
...
for n in range(1, num_trades + 1):
    ...
    L_n = (n - cumulative_loss) / 98.5

    # Calculate outcomes
    win_amount = 98.5 * L_n
    lose_amount = -101.5 * L_n
    ...
    data.append({
        ...
        "If Win (98.5×L)": f"${win_amount:,.4f}",
        "Cumulative Profit if Win at This Trade": f"${n}",
        "If Lose (-101.5×L)": f"${lose_amount:,.4f}",
